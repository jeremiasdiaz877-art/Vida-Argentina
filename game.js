...
function procesarTurno(dado) {
  ...
  j._gananciaTurno = ingresoSueldo + retornoEmpresa - gastosFijos - costoCarrera;
  ...
  if (Math.random() < 0.6) {
    ...
    j._gananciaTurno += impactoEvento;
  }

  // Impuesto a las ganancias (sobre la ganancia del período, ya incluye el evento)
  ...
  j._resumen = {
    ...
    evento, impactoEvento, impuesto, saldoDespues: j.saldo
  };
...
function mostrarResumenTurno(j) {
  ...
  html += `<div style="background:var(--gris);border-radius:12px;padding:14px;margin-bottom:16px;">
    ...
    <table style="width:100%;font-size:14px;">
      ...
      ${linea("🧾 Impuesto a las Ganancias", r.impuesto, true)}
...
function abrirPrestamos() {
  ...
  function actualizarResumenPrestamo() {
    ...
    // Tope de endeudamiento: el capital total no puede superar 12x el sueldo mensual
    const j = estado.jugadores[estado.jugadorActual];
    const deudaActual = j.prestamos.reduce((s, p) => s + p.principal, 0);
    const topeDeuda = j.sueldo * 12;
    if (deudaActual + monto > topeDeuda) {
      ...
      cont.innerHTML = `<div class="prestamo-resumen" style="background:#f8d7da;border-color:#f5c6cb;">
        ...
        Deuda actual (capital): ${fmt(deudaActual)}<br>
        Margen disponible: ${fmt(disponible)}
      </div>`;
      return;
    }
    ...
    cont.innerHTML = `
      <div class="prestamo-resumen">
        ...
        Interés total: ${fmt(interesTotal)} • Total: ${fmt(totalAPagar)}<br>
        <span style="color:var(--gris-dark);font-size:12px;">Tope de deuda (12× sueldo): ${fmt(topeDeuda)} • Disponible tras pedir: ${fmt(topeDeuda - deudaActual - monto)}</span>
      </div>
    `;
